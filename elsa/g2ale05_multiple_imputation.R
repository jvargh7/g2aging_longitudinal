...
proportion_vars <- c(paste0(rep(c("w_","h_"),each=11),
                            c("diagnosed_bp","medication_bp",
                              ...
                              "smoke","heavydrinker","moderate_pa","vigorous_pa",
                              "insurance","htn")),"hh_lengthmar_ge10","w_htn") #"residence",
...
id_vars = c("w_personid","h_personid","w_spouseid","h_spouseid","coupleid","h_sampleweight", 
            "w_sampleweight","strata","psu","wave","h_normalizedweight","w_normalizedweight")
...
before_imputation %>% 
  dplyr::select(-one_of(c("coupleid","w_personid","h_personid","h_sampleweight","w_sampleweight","strata","psu"))) %>% 
...
saveRDS(mi_dfs, paste0(path_g2a_family_folder,"/elsa/G2A ELSA Couples mi_dfs.RDS"))